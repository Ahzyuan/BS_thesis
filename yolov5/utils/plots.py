...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    box_color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3])) #bdbox左上角和右下角坐标
    cv2.rectangle(img, c1, c2, box_color, thickness=tl, lineType=cv2.LINE_AA)   
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]   #(fw,fh)
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, box_color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=1, lineType=cv2.LINE_AA)
        
def plot_one_box_custom(x, img, color=None, label=None, line_thickness=3):
    # Plots one bounding box on image img
    tl = 1#line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    box_color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3])) #bdbox左上角和右下角坐标   
    if label:
        ...
        cv2.line(img, (img.shape[1]//2, img.shape[0]), ((c1[0]+c2[0])//2,c2[1]), (0, 0, 255))
