# borrowed from https://github.com/davidjaw/Domain-adaptation-object-detection-with-YOLOv8.git
...
import numpy as np
import torch,time,argparse,warnings,random,os,shutil
...
class CustomTrainer(DetectionTrainer):
    ...
    def __init__(self, target_domain_data_cfg, *args, **kwargs):
        ...
        self.t_trainset, self.t_testset = self.t_data["train"], self.t_data.get("val") or self.t_data.get("test") # self.get_dataset(self.t_data)
        self.t_iter = None
    ...
    def get_dis_output_from_hooked_features(self, batch):
        ...
        if self.model_hooked_features is not None:
            ...
            for fidx, f in enumerate(self.model_hooked_features):
                ...
                f_roi = roi_align(f.to(f_bbox.device), f_bbox, output_size=self.roi_size[fidx], aligned=True)
                rois.append(f_roi.to(f.device))
...
def main(args):
    ...
    del kwargs['td_cfg']
    del kwargs['sd_cfg']
    del kwargs['enable_da']
    del kwargs['enable_train']

    seed_everything(args.seed)

    model = YOLO(args.weight)
    source_domain = args.sd_cfg.split(os.path.sep)[-2]
    target_domain = args.td_cfg.split(os.path.sep)[-2]
    domain_info = f'S({source_domain.lower()})-T({target_domain.lower()})'

    if args.enable_da:
        custom_trainer = partial(CustomTrainer, target_domain_data_cfg=args.td_cfg)
    else:
        custom_trainer = None
    
    if args.enable_train:
        model.train(custom_trainer, data=args.sd_cfg, name=f'train_{domain_info}', patience=50, **kwargs)

    t_metrics = model.val(data=args.td_cfg, name=f'val_on_target(train-{domain_info})')
    s_metrics = model.val(data=args.sd_cfg, name=f'val_on_source(train-{domain_info})')

    for metrics in [t_metrics, s_metrics]:
        log_path = os.path.join(metrics.save_dir, 'res.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--weight', required=True, default='yolov8s.pt', type=str, help='Pretrained weight path')
    parser.add_argument('-t', '--td_cfg', required=True, default='../Dataset/TPZ_VOCdevkit/d435/TPZ.yaml', type=str, help='Target domain data config path')
    parser.add_argument('-s', '--sd_cfg', required=True, default='../Dataset/TPZ_VOCdevkit/d2city/TPZ.yaml', type=str, help='Source domain data config path')
    parser.add_argument('--enable_da', action='store_true', help='whether enable domain adaptation')
    parser.add_argument('--enable_train', action='store_true', help='whether to train')

    parser.add_argument('-n', '--workers', default=8, type=int, help='Maximum number of dataloader workers')
    parser.add_argument('-d', '--device', default='0', type=str, help='Specifies the computational device(s) for training: a single GPU (device=0), multiple GPUs (device=0,1), CPU (device=cpu), or MPS for Apple silicon (device=mps).')
    
    #parser.add_argument('--project', default=None, type=str, help='Name of the project directory where training outputs are saved. Allows for organized storage of different experiments.')
    #parser.add_argument('--name', default=None, type=str, help='Name of the training run. Used for creating a subdirectory within the project folder, where training logs and outputs are stored.')
    ...
    parser.add_argument('--batch', default=16, type=int, help='total batch size for all GPUs')
    parser.add_argument('--seed', default=2565, type=int, help='random seed')
    parser.add_argument('--resume', action='store_true', help='Resumes training from the last saved checkpoint. Automatically loads model weights, optimizer state, and epoch count, continuing training seamlessly.')
    parser.add_argument('--val', action='store_true', help='Enables validation during training, allowing for periodic evaluation of model performance on a separate dataset.')
    parser.add_argument('--profile', action='store_true', help='Enables profiling of ONNX and TensorRT speeds during training, useful for optimizing model deployment.')
    parser.add_argument('--amp', action='store_true', help='Enables Automatic Mixed Precision (AMP) training, reducing memory usage and possibly speeding up training with minimal impact on accuracy.')

    parser.add_argument('--save_period', default=50, type=int, help='Frequency of saving model checkpoints, specified in epochs. A value of -1 disables this feature. Useful for saving interim models during long training sessions.')
    parser.add_argument('--plots', action='store_true', help='Generates and saves plots of training and validation metrics, as well as prediction examples, providing visual insights into model performance and learning progression.')

    args = parser.parse_args()
    args.weight = os.path.abspath(args.weight)
    args.td_cfg = os.path.abspath(args.td_cfg)
    args.sd_cfg = os.path.abspath(args.sd_cfg)
    assert os.path.exists(args.weight), f'{args.weight} not found'
    assert os.path.exists(args.td_cfg), f'{args.td_cfg} not found'
    assert os.path.exists(args.sd_cfg), f'{args.sd_cfg} not found'
